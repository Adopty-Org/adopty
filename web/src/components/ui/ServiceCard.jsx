  ...
  return (
    <FadeIn delay={delay} className="bg-surface-container-lowest border-4 border-black rounded-xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(21,66,18,1)] group hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[12px_12px_0px_0px_rgba(21,66,18,1)] transition-all duration-200">
      <div className="p-6">
        ...
        <p className="text-sm text-on-surface-variant mb-4 leading-relaxed line-clamp-2">{prestataire?.Bio}</p>

        <div className="flex flex-wrap gap-1.5 mb-4">
          {prestataire?.especes?.map(a => (
            <span key={a?.Id} className="text-xs font-bold bg-surface-variant px-2 py-0.5 border border-black/20 rounded-sm uppercase tracking-tight">{a?.Nom}</span>
          ))}
        </div>

        <div className="flex items-center justify-between border-t-2 border-black/10 pt-4">
          <div>
            <span className="font-['Plus_Jakarta_Sans'] font-extrabold text-2xl text-primary">{prestataire?.TarifHoraire}€</span>
