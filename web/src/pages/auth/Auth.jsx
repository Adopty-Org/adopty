...
// Auth.jsx - Version avec création UNIQUEMENT à la dernière étape
...
const STEPS_SIGNUP = ['Informations', 'Rôle', 'Compte']
...
const Auth = () => {
  ...
  // Étape 1: Sauvegarde des infos perso en mémoire
  const handleStep0 = async (e) => {
    e.preventDefault()
    setStep(1)
  }

  // Étape 2: Sauvegarde du rôle en mémoire
  const handleStep1 = async (e) => {
    e.preventDefault()
    if (!role) {
  ...
  // ✅ Étape 3: Création COMPLÈTE du compte Clerk + envoi à ta base de données
  const handleFinalize = async (e) => {
    e.preventDefault()
    ...
    if (!signUpLoaded) {
      setError('Le système n\'est pas prêt, veuillez rafraîchir')
      return
    }
    ...
    try {
      // 1. Créer le compte Clerk avec TOUTES les métadonnées d'un coup
      const metadata = {
        prenom,
        nom,
        wilaya,
      ...
      console.log("🟡 Création du compte avec:", { email, role })

      // Création du compte Clerk avec vérification email automatique
      const result = await signUp.create({
        emailAddress: email,
        password,
        //phoneNumber: telephoneUtilisateur,
        unsafeMetadata: metadata
      })

      console.log("🟢 Compte créé:", result.status)

      // ✅ CRUCIAL - Log ce qui manque
    console.log("📋 État après création:", {
      ...
      passwordSet: !!signUp.password
    })

      // 2. Envoyer le code de vérification par email
      console.log("📧 Envoi du code de vérification...")
      await signUp.prepareEmailAddressVerification({ strategy: 'email_code' })

      // ✅ Log après envoi
    console.log("📋 État après préparation:", {
      status: signUp.status,
      requiredFields: signUp.requiredFields,
      missingFields: signUp.missingFields,
      unverifiedFields: signUp.unverifiedFields
      ...
      // 3. Passer en mode vérification
      setPendingVerification(true)
      console.log("✅ Code envoyé, passage en mode vérification")
      setLoading(false)
  ...
  const handleVerify = async (e) => {
  e.preventDefault()
  console.log("🔵 Début vérification, code:", verificationCode)
  
  if (!signUpLoaded) {
    console.log("❌ signUp pas chargé")
    return
  }
  ...
  try {
    ...
    if (result.status === 'complete') {
      console.log("✅ Vérification complète, création de la session...")
      await setActiveSignUp({ session: result.createdSessionId })
      ...
      console.log("❌ Status inattendu:", result.status)
      setError('Code invalide. Vérifiez votre email.')
    }
  } catch (err) {
    console.error("🔴 Erreur détaillée:", err)
    console.log("Message d'erreur:", err.errors?.[0]?.longMessage)
    setError(err.errors?.[0]?.longMessage || 'Code invalide.')
  } finally {
    setLoading(false)
  }
    ...
    useEffect(() => {
    if (signUp) {
        console.log("État de signUp:", {
        status: signUp.status,
        hasSession: !!signUp.createdSessionId,
...
}, [pendingVerification, step, verificationCode, signUpLoaded, signUp])
// Juste avant le return, à la fin du composant Auth
console.log("🔍 Vérification que handleVerify existe:", typeof handleVerify)
      ...
      <div className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-md py-8">
          ...
          <AnimatePresence mode="wait">
            ...
            {/* SIGNUP - Création à la dernière étape */}
            {mode === 'signup' && (
              <motion.div key="signup" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <div className="mb-6">
                  <h1 className="font-['Chewy'] text-4xl text-primary mb-1">
                    {step === 0 ? 'Vos informations' : step === 1 ? 'Votre rôle' : 'Création du compte'}
                  </h1>
                  <p className="text-on-surface-variant text-sm">Étape {step + 1} sur {STEPS_SIGNUP.length}</p>
                  <div className="flex gap-1.5 mt-3">
                    {STEPS_SIGNUP.map((_, i) => (
                ...
                {step === 0 && !pendingVerification && (
                  <form onSubmit={handleStep0} className="space-y-4">
                    ...
                    <InputField label="Wilaya" value={wilaya} onChange={setWilaya} required />
                    <InputField label="Adresse" value={adresse} onChange={setAdresse} required />
                    <InputField label="Telephone" value={telephoneUtilisateur} onChange={setTelephoneUtilisateur} required />
                    {error && <p className="text-error text-sm font-bold bg-error-container px-4 py-3 rounded-lg border border-error">{error}</p>}
                    <button type="submit"
                      className="w-full py-4 bg-primary text-white font-['Plus_Jakarta_Sans'] font-extrabold text-sm uppercase tracking-widest border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all">
                      Continuer →
                ...
                {step === 2 && !pendingVerification && (
                  <form onSubmit={handleFinalize} className="space-y-4">
                    <InputField label="Email" type="email" value={email} onChange={setEmail} required />
                    <InputField label="Mot de passe" type="password" value={password} onChange={setPassword} required />
                    <InputField label="Confirmer mot de passe" type="password" value={confirmPassword} onChange={setConfirmPassword} required />
                ...
                {pendingVerification && (
                  <form onSubmit={handleVerify} className="space-y-4">
                    <div className="bg-primary-fixed border-2 border-black rounded-xl p-4 text-center">
                      <span className="material-symbols-outlined text-primary text-3xl mb-2 block">mark_email_read</span>
                      <p className="font-bold text-sm">Un code a été envoyé à <strong>{email}</strong></p>
                      <p className="text-xs mt-2">Vérifiez vos spams si vous ne le recevez pas</p>
                    </div>
                    <InputField label="Code de vérification" value={verificationCode} onChange={setVerificationCode} required />
                    {error && <p className="text-error text-sm font-bold bg-error-container px-4 py-3 rounded-lg border border-error">{error}</p>}
                    <button type="submit" disabled={loading}
                      className="w-full py-4 bg-primary text-white font-['Plus_Jakarta_Sans'] font-extrabold text-sm uppercase tracking-widest border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all">
                      {loading ? 'Vérification...' : 'Vérifier et finaliser →'}
                    </button>
                  </form>
                )}
              </motion.div>
